...
#
# Dispatch the single vgi-cbor image into one of its transports:
...
#
# The worker is stateless (pure in-engine codec), so there is no /data to create
...
case "${1:-http}" in
  http)
    ...
    exec cbor-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec cbor-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec cbor-worker "$@"
