...
LIB="target/$TARGET/release/libcbor_wasm.a"
...
#
# MALLOC=mimalloc is load-bearing for multithreaded serve: emscripten's default
...
# which the Arrow encoders + the SDK transport can overflow — an overflow surfaces
# as a bare "memory access out of bounds" with no console output. INITIAL_MEMORY
