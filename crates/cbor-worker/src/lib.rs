...
//! the `cbor-wasm` crate (the browser build, which serves the same `Worker` over
...
fn catalog_metadata(name: &str) -> CatalogModel {
        ...
        tags: vec![
            ...
            (
                ...
                "Read, write, and reshape CBOR (RFC 8949) and MessagePack binary payloads from \
                 ...
                 and you need to inspect, validate, flatten, or re-encode them."
                    .to_string(),
            ),
            (
                "vgi.doc_md".to_string(),
            ...
            // VGI509: at least one guaranteed-runnable, verified example. These
            // need no external file or backend, so they execute cleanly in the
            // lint sandbox and double as a codec round-trip smoke test.
            (
  ...
  },
  {
    ...
    "expected_result": [{"alg": "ES256"}]
        ...
        // Worker build version, surfaced via `vgi_catalogs()` / `duckdb_databases()`
        // so a caller reads it without spending a query — the discoverable
        // replacement for a parameterless `cbor_version()` scalar (VGI328).
        implementation_version: Some(cbor_core::version().to_string()),
        schemas: vec![CatSchema {
            ...
            tags: vec![
                ...
                (
                    "vgi.example_queries".to_string(),
                    r#"[{"description":"Render a CBOR array blob as JSON text.","sql":"SELECT cbor.main.to_json(from_hex('83010203')) AS json"},
                       {"description":"Show a tag-1 epoch timestamp in human-readable diagnostic (EDN) notation.","sql":"SELECT cbor.main.diagnostic(from_hex('c11a514b67b0')) AS edn"},
                       {"description":"Encode a JSON array into deterministic CBOR and hex it.","sql":"SELECT to_hex(cbor.main.from_json('[1,2,3]')) AS hex"},
                       {"description":"Check that an untrusted blob is well-formed CBOR.","sql":"SELECT (cbor.main.well_formed(from_hex('83010203'))).ok AS ok"},
                       {"description":"Pull the issuer claim out of a bare CWT claim set.","sql":"SELECT (cbor.main.cwt_claims(from_hex('a10169636f61703a2f2f6173'))).iss AS iss"},
                       {"description":"Fan a 3-item CBOR Sequence into one row per item.","sql":"SELECT idx, value FROM cbor.main.seq_decode(from_hex('010203')) ORDER BY idx"}]"#
                        .to_string(),
                ),
            ],
...
/// The catalog name DuckDB sees in `ATTACH 'cbor' (TYPE vgi, …)`. Defaults to
/// `cbor`, but honors an explicit override so a test harness can rename it.
