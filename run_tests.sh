#!/usr/bin/env bash
# Build the cbor VGI worker and run the SQLLogic tests against it using the
...
echo "==> Building cbor-worker (release)"
cargo build --release --bin cbor-worker
...
WORKER="$REPO_ROOT/target/release/cbor-worker"
