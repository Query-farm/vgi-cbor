...
#
# Single image that serves every network transport of the `cbor` VGI worker:
...
#
# This worker is STATELESS: the CBOR / MessagePack / COSE / CWT / WebAuthn
# decoders are pure in-engine compute over a BLOB column, with no network, no
# persisted state, and zero egress. So there is no /data volume, no state
...
# runtime ships. The SDK comes from crates.io (no git/path dependency), so a
# plain registry fetch is all the build needs.
...
# to a non-cache path before the layer ends (cache mounts don't persist in the
# image).
COPY Cargo.toml Cargo.lock ./
COPY crates ./crates
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    --mount=type=cache,target=/src/target \
    cargo build --release --locked --bin cbor-worker \
    && cp target/release/cbor-worker /cbor-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-cbor
...
LABEL org.opencontainers.image.title="vgi-cbor" \
      org.opencontainers.image.description="CBOR (RFC 8949) / MessagePack decode & encode plus tag-aware COSE / CWT / WebAuthn structural decode as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_CBOR_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /cbor-worker /usr/local/bin/cbor-worker
